# 국회 · 행정부 에너지 정책 성향 분석 대시보드

SK E&S 관점에서 **22대 국회의원 + 국무위원**의 에너지(LNG·수소·재생E·도시가스·전력) 성향을
육각 카토그램 지도·프로필·근거 발언으로 탐색하는 웹 대시보드. (기획안 v1.0 기반)

> **현재 상태**: MVP 화면이 **가상 샘플 데이터**로 동작합니다. `.env` 에 키를 넣고 파이프라인을 실행하면 실데이터로 바뀝니다.

## 빠른 시작 (지금 바로 화면 보기)

```bash
cd assembly-dashboard
npm run serve      # → http://localhost:8137  (Node만 있으면 됨, 설치 패키지 없음)
```
...
- 추후 Next.js 로 이전 시: `web/` 의 화면 로직을 컴포넌트로 옮기고 `data/*.json` 을 import.
