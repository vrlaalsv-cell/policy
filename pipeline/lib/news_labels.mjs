...
export const NEWS_LABELS = [
  ...
  },
  {
    ...
    //   ① 全力(전력을 다함)  ② 戰力(군사력)  ③ 前歷(과거 경력) — 셋 다 電力이 아니다.
    neg: new RegExp([
      "전심\\s*전력", "전력\\s*질주", "전력\\s*투구", "전력을?\\s*다\\s*해?", "전력해", "전력 다\\b", "총력전", "사력을",
      "전력\\s*(증강|공백|누수|보강|손실|열세|우위|평가전|요소)",
      "(군|해병대|해군|공군|육군|방산|국방|전투)\\s*전력", "전력화", "즉시\\s*전력",
      // 前歷: "뺑소니 전력", "변호 전력", "전력 지적"
      "(뺑소니|범죄|음주|전과|비위|폭행|처벌|변호|수사|기소|문제)\\s*전력", "전력\\s*(지적|논란)",
      // 全力: "회복에 전력", "지선에 전력" 처럼 '~에 전력'으로 문장이 끝나는 형태.
      //   뒤에 전력망·수급·공급 등이 오면 진짜 電力이므로 제외하지 않는다.
      "에\\s*['\"“‘]?전력['\"”’]?\\s*(?=[.,…\\]\\)]|$)",
    ].join("|"), "g"),
  },
];
