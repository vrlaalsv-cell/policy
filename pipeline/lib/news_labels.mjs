// 기사 → 에너지원 라벨 분류 (LNG·수소·원전·재생E·도시가스·전력·에너지솔루션·분산에너지)
...
export const NEWS_LABELS = [
  ...
  {
    id: "DISTE", label: "분산에너지", color: "#be185d", bg: "#fce7f0",
    kw: ["분산에너지", "분산 에너지", "분산특구", "분산 특구", "특화지역", "지산지소", "직접전력거래", "전력직구", "지역별 차등요금", "지역 차등요금"],
  },
  {
    id: "ESOL", label: "에너지솔루션", color: "#0369a1", bg: "#e3f2fb",
    // ⚠ "ESS" 단독 키워드는 넣지 않는다 — Latin 3글자라 Express·Business·Success 같은 영문 혼용 기사에
    //   오탐될 수 있다("전력"의 全力/戰力 오탐과 같은 부류 함정). 한글 전체어("에너지저장장치")로만 잡는다.
    kw: ["에너지솔루션", "에너지 솔루션", "집단에너지", "열병합", "가상발전소", "VPP", "수요반응", "통합에너지", "에너지저장장치"],
  },
  {
...
const OTHER_ROLE = "(교수|박사|연구원|연구위원|총장|학장|코치|선수|감독|본부장|팀장|아나운서|가수|배우|작가|셰프|주무관|대표|사장|회장|이사장|CEO|상임이사)";
//   '공약·후보·선거'도 정치인 단서다 — 이게 없으면 "용혜인 …공약 발표" 같은 기사가 '대표'에 걸려 날아간다.
const POLITICIAN_RE = /의원|장관|국회|의장|위원장|당대표|최고위원|원내대표|더불어민주당|국민의힘|조국혁신당|개혁신당|진보당|기본소득당|사회민주당|무소속|대통령실|정부|시의회|도의회|공약|후보|선거/;

/** 이름은 같지만 다른 사람(교수·기업 대표·선수 등)의 기사인가
 *  이름과 직함 사이에 소속이 끼는 형태까지 본다 — "김준환 스트라드비젼 대표", "김종민·장병용 상임이사". */
export function isOtherPerson(text, name) {
  if (!name) return false;
  const t = String(text || "");
  return new RegExp(name + "[가-힣A-Za-z0-9·\\s]{0,12}" + OTHER_ROLE).test(t) && !POLITICIAN_RE.test(t);
}
...
export const QUERY_TERMS = ["LNG", "수소", "원전", "재생에너지", "도시가스", "전력", "에너지솔루션", "분산에너지"];
