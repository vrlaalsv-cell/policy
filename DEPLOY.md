...
### ① 기사 자동 재수집 + 회의록 원본 확보 — 등록할 것 없음

`docker compose up -d` 하면 `policy-scheduler` 컨테이너가 같이 뜬다. **두 가지를 자동으로 한다.**
...
**국무·차관회의 회의록 PDF 확보 (2026-08-12 추가)**
- **매일 05:30 KST + 컨테이너 시작 시** 행안부 게시판을 훑어 새 회의록 PDF 를 받는다. 이미 받은 건 건너뛴다.
- **왜 자동화했나** — 새 회의록을 놓치지 않고 제때 확보하기 위해서다. 회의록은 회의 후 약 6주 뒤 올라와
  주기가 불규칙하고, 사람이 기억해서 돌리면 놓치기 쉽다.
  ⚠️ (2026-08-12 정정) 한때 "행안부가 옛 첨부를 내린다"고 적었으나 **오판이었다** — 실제 원인은
  **WAF 가 연속 요청을 차단**하는 것이었고, 간격·재시도를 넣으니 2023년치까지 전부 받아진다.
- **받기만 한다.** AI 판정·빌드는 사람이 PC 에서 한다(비용이 드는 단계라 일부러 자동화하지 않았다).
- PDF 는 `./runtime/cabinet_raw/`(컨테이너 `/app/data/cabinet_raw`)에 쌓인다 — **볼륨이라 재빌드해도 남는다.**
- 끄려면 compose 의 `CABINET_FETCH=0`. 시각은 `CABINET_AT`, 훑을 목록 페이지 수는 `CABINET_PAGES`.
- ⚠️ **NAS 가 받은 PDF 는 PC 로 가져와야 판정할 수 있다**(판정은 PC 에서 함).
  File Station 이나 `scp` 로 `runtime/cabinet_raw/` → `C:\AI\_corpus\cabinet_raw\` 로 주기적으로 내려받을 것.
...
`가동 — 매일 06:00, 09:00, 12:00, 15:00, 18:00 (KST) 기사 수집` 과 그 다음 줄
`회의록 PDF 확보 매일 05:30 (KST)` 이 같이 보이면 정상(시각은 `docker-compose.yml` 의
