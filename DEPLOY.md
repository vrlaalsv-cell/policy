...
- **하루 3번(07:00 / 13:00 / 19:00 KST)** `collect:news → fetch:excerpts` 를 자동 실행
- 컨테이너가 **시작될 때** 데이터가 8시간 넘게 낡았으면 즉시 1회 수집 (재빌드 직후 공백 방지)
...
횟수를 늘리려면 `docker-compose.yml` 의 `NEWS_AT` 에 시각을 쉼표로 더 넣으면 된다:

```yaml
- NEWS_AT=07:00,10:00,13:00,16:00,19:00
```

> 다만 하루 10회 이상은 권하지 않는다. 실측상 **에너지 관련 새 기사는 하루 10건 안팎**이라 얻는 게 거의 없고,
> Google 쪽에서 요청 폭주로 차단당할 위험만 커진다.
